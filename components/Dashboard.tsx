...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      {/* ── Filters ── */}
      <div className="px-5 sm:px-7 lg:px-8 py-6" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
      ...
      <div className="flex-1 overflow-y-auto px-5 sm:px-7 lg:px-8 py-8">
        <div className="max-w-6xl space-y-10">
          {/* ── KPIs — ruled 3-cell strip, no boxes ── */}
          <section>
            <SectionLabel accent="var(--accent-primary)">Resumen</SectionLabel>
            <div className="grid grid-cols-3 mt-5" style={{ borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
              {kpis.map((kpi, i) => (
                <div key={i} className="py-6" style={{ borderLeft: i > 0 ? '1px solid var(--border-subtle)' : 'none', paddingLeft: i > 0 ? '24px' : 0 }}>
                  <Display size="lg" color={kpi.color} style={{ fontFamily: 'var(--font-display)', fontVariantNumeric: 'tabular-nums' }}>{kpi.value}</Display>
                  <p className="mt-2.5" style={{ fontFamily: 'var(--font-heading)', fontSize: '10px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>{kpi.label}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            {alerts.length === 0 ? (
              ...
              <div className="mt-2">{alerts.map((a, i) => <AlertRow key={a.id} alert={a} first={i === 0} onDelete={() => handleDeleteAlert(a.id)} />)}</div>
            )}
          </section>

          {/* ── Charts — each in a corner-cut Panel ── */}
          <section>
            <Panel cut="sm" className="p-5 sm:p-6 lg:p-8">
              <Display size="md" upper>Tendencia de consultas</Display>
              <div className="h-48 sm:h-64 mt-5"><Line data={trendData} options={chartOptions} /></div>
            </Panel>
          </section>

          <section className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6 lg:gap-8">
            <Panel cut="sm" className="p-5 sm:p-6 lg:p-8">
              <Display size="md" upper>Por categoría</Display>
              <div className="h-48 sm:h-64 mt-5"><Doughnut data={categoryData} options={doughnutOptions} /></div>
            </Panel>
            <Panel cut="sm" className="p-5 sm:p-6 lg:p-8">
              <Display size="md" upper>Más consultadas</Display>
              <div className="h-48 sm:h-64 mt-5"><Bar data={topSubstancesData} options={chartOptions} /></div>
            </Panel>
          </section>
